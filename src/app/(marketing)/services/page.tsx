    ...
    return (
        <div className="bg-black min-h-screen pt-32 pb-24">
            ...
            <section className="container mx-auto px-4 mb-20 relative z-10">
                <RevealOnScroll>
                    <div className="text-center max-w-3xl mx-auto">
                        <h1 className="font-heading text-5xl md:text-6xl font-bold text-white mb-6">Premium London Cleaning Services</h1>
                        <p className="text-white/40 text-lg">Comprehensive exterior window and property maintenance tailored to the exact needs of London's residential and commercial properties.</p>
                    </div>
                </RevealOnScroll>
            </section>

            {/* Services List */}
            <section className="container mx-auto px-4 relative z-10 space-y-8">
                {services.map((service, index) => (
                    <RevealOnScroll key={service.id} delay={index * 0.1}>
                        <div className="bg-[#111111]/50 backdrop-blur-sm border border-white/5 rounded-3xl p-8 lg:p-12 hover:bg-[#111111] hover:border-[#FFD700]/30 transition-all group">
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center">

                                {/* Icon & Title */}
                                <div className="lg:col-span-4 flex flex-col items-start">
                                    <div className="w-16 h-16 rounded-2xl bg-[#FFD700]/10 flex items-center justify-center mb-6 group-hover:bg-[#FFD700] group-hover:scale-110 transition-all duration-300">
                                        <service.icon className="w-8 h-8 text-[#FFD700] group-hover:text-[#0A1628] transition-colors" />
                                    </div>
                                    <h2 className="font-heading text-3xl font-bold text-white mb-4">{service.title}</h2>
                                ...
                                <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
                                    <div>
                                        <p className="text-white/40 text-lg leading-relaxed mb-6">{service.description}</p>
            ...
            <section className="container mx-auto px-4 mt-24">
                <RevealOnScroll>
                    <div className="bg-gradient-to-r from-[#1B2A4A] to-[#0A1628] border border-[#FFD700]/20 rounded-3xl p-12 text-center max-w-4xl mx-auto shadow-[0_0_50px_rgba(0,212,255,0.05)]">
                        <h3 className="font-heading text-3xl md:text-4xl font-bold text-white mb-6">Not sure what you need?</h3>
                        <p className="text-white/40 text-lg mb-8 max-w-2xl mx-auto">Build a custom package and get an exact price estimate instantly using our smart quoting tool.</p>
                        <Link href="/quote" className="inline-flex items-center justify-center gap-2 px-10 py-5 bg-[#FFD700] hover:bg-[#FFC107] text-[#0A1628] font-bold rounded-full transition-all hover:scale-105 shadow-[0_0_20px_rgba(0,212,255,0.3)]">
                            Start Instant Quote
