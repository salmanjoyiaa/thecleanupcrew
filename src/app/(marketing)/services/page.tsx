    ...
    return (
        <div className="bg-[#F8F6F0] min-h-screen pt-24 md:pt-32 pb-16 md:pb-24 relative">
            ...
            <section className="container mx-auto px-5 md:px-4 mb-12 md:mb-20 relative z-10">
                <RevealOnScroll>
                    <div className="text-center max-w-3xl mx-auto rounded-2xl border border-black/10 bg-white/80 p-6 md:p-8 shadow-sm">
                        <p className="text-xs font-semibold uppercase tracking-[0.2em] text-[#9A7B00] mb-3">Service Catalog</p>
                        <h1 className="font-heading text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-bold text-[#1A1A1A] mb-4 md:mb-6 leading-tight">Premium London Cleaning Services</h1>
                        <p className="text-muted-foreground text-base md:text-lg">Comprehensive exterior window and property maintenance tailored to the exact needs of London&apos;s residential and commercial properties.</p>
                    </div>
                </RevealOnScroll>
            </section>
            ...
            <section className="container mx-auto px-5 md:px-4 relative z-10 space-y-5 md:space-y-8">
                {services.map((service, index) => (
                    <RevealOnScroll key={service.id} delay={index * 0.1}>
                        <div className="bg-white/80 backdrop-blur-sm border border-black/10 rounded-2xl md:rounded-3xl p-6 md:p-8 lg:p-12 hover:bg-white hover:border-[#FFD700]/30 transition-all group">
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-8 lg:gap-12 items-center">
                                ...
                                <div className="lg:col-span-4 flex flex-col items-start">
                                    ...
                                    <h2 className="font-heading text-2xl md:text-3xl font-bold text-[#1A1A1A] mb-3 md:mb-4">{service.title}</h2>
                                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-black/5 border border-black/10 text-[#C99600] font-medium font-accent text-sm">
                                        {service.price}
                                    </div>
                                </div>
                                ...
                                <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 lg:gap-12">
                                    <div>
                                        <p className="text-muted-foreground text-base md:text-lg leading-relaxed mb-4 md:mb-6">{service.description}</p>
                                        <Link href={`/services/${service.id}`} className="inline-flex items-center gap-2 text-[#9A7B00] hover:text-foreground font-medium transition-colors border-b border-[#FFD700]/30 hover:border-[#1A1A1A]/30 pb-1 w-fit">
                                            View full details <ArrowRight className="w-4 h-4" />
                                        </Link>
                                    </div>

                                    <div className="bg-[#FCFAF3] rounded-2xl p-5 md:p-6 border border-black/10">
                                        <h4 className="text-[#1A1A1A] font-medium mb-3 md:mb-4 text-sm md:text-base">What&apos;s Included:</h4>
            ...
            <section className="container mx-auto px-5 md:px-4 mt-16 md:mt-24">
                <RevealOnScroll>
                    <div className="bg-gradient-to-r from-[#1B2A4A] to-[#0A1628] border border-[#FFD700]/20 rounded-2xl md:rounded-3xl p-8 md:p-12 text-center max-w-4xl mx-auto shadow-[0_0_50px_rgba(0,212,255,0.05)]">
                        ...
                        <p className="text-white/80 text-base md:text-lg mb-6 md:mb-8 max-w-2xl mx-auto">Build a custom package and get an exact price estimate instantly using our smart quoting tool.</p>
